...
A single topic is used:

\begin{itemize}
  \item \textbf{\texttt{raw\_h2\_data}} — the sole Kafka topic.
        Every message published by the producer lands here.
        The consumer reads from this topic, applies quality checks, and
        writes results directly to QuestDB.
        \texttt{validated\_h2\_data} is a QuestDB \emph{table} name, not a
        second Kafka topic.
\end{itemize}
...
Four tables are maintained at runtime (Table~\ref{tab:tables}).
